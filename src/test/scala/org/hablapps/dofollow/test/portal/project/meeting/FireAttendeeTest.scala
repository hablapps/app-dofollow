...
class FireAttendeeTest(System: speech.System with DoFollowSystem with react.Debug) extends FunSpec with ShouldMatchers with BeforeAndAfter {
	describe( "FireAttendeeTest"){
		it("Fire Attendee") {

		  import System._
		
		  val Output(meeting1, attendee1, moderator1) = reset(for {
		    ...
		    attendee1 			<- Play3(Attendee(_persistent = true), operator1, meeting1)
		  } yield (meeting1, attendee1, moderator1))
		
		  val NextState(obtained) = attempt(Say(moderator1, meeting1, FireAttendee(_role = Some(attendee1))))
		  ...
		  reset(for {
		    ...
		    moderator1 			<- Play3(Moderator(_persistent = true), admin1, meeting1)
		    attendee1 			<- Play3(Attendee(_persistent = true), operator1, meeting1)
		    fire1 				<- Say(moderator1, meeting1, FireAttendee(_role = Some(attendee1)))
		    _ 					<- Done(fire1, PERFORMED)
		    _ 					<- Abandon(attendee1)
		  } yield ())
