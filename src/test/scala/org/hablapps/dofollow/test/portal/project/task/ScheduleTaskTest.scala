...
class ScheduleTaskTest(System: speech.System with DoFollowSystem with react.Debug) extends FunSpec with ShouldMatchers with BeforeAndAfter {
	describe( "ScheduleTaskTest"){
		it("Schedulinkg Task") {

		  import System._
		
		  val Output(task1, admin1) = reset(for {
		  ...
		  val NextState(obtained) = attempt(Say(admin1, task1, ScheduleTask(_date = 123, _reason = "Reason 1")))
		  ...
		  reset(for {
		    ...
		    _ 					<- Let(dep1.subinteraction += task1)
		    responsible1 		<- Play3(Responsible(_persistent = true), admin1, task1)
		    speechAct1 			<- Say(admin1, task1, ScheduleTask(_date = 123, _reason = "Reason 1"))
		    _ 					<- LetWholeExtension(speechAct1, "persistent", true)
		    _ 					<- Perform(speechAct1)
		    // _ 					<- LetWholeExtension(task1, "deadline", 123)
		    // _ 					<- Let(task1, "estimatedDate", 123, true)
		    // _ 					<- Let(task1, "reason", "Reason 1", true)
		  } yield ())
