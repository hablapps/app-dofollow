...
class LeaveInviteeTest(System: speech.System with DoFollowSystem with react.Debug) extends FunSpec with ShouldMatchers with BeforeAndAfter {
	describe( "LeaveInviteeTest"){
		it("Leave Invitee") {

		  import System._
		
		  val Output(meeting1, invitee1, moderator1) = reset(for {
		  ...
		  } yield (meeting1, invitee1, moderator1))
		
		  val NextState(obtained) = attempt(Say(moderator1, meeting1, FireInvitee(_role = Some(invitee1))))
		  ...
		  reset(for {
		    ...
		    moderator1 			<- Play3(Moderator(_persistent = true), admin1, meeting1)
		    invitee1 			<- Play3(Invitee(_persistent = true), operator1, meeting1)
		    fire1 				<- Say(moderator1, meeting1, FireInvitee(_role = Some(invitee1)))
