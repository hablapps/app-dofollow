...
class JoinAttendeeTest(System: speech.System with DoFollowSystem with react.Debug) extends FunSpec with ShouldMatchers with BeforeAndAfter {
	describe( "JoinAttendeeTest"){
		it("Join Attendee in a scheduled meeting") {

		  import System._
		
		  val Output(meeting1, operator2) = reset(for {
		    ...
		    operator2 			<- Play2(Operator(_persistent = true, _substatus = Some(Enable), _name = Some("34")), dep1)
		  ...
		  val NextState(obtained) = attempt(Say(operator2, meeting1, JoinAttendee(__new = Some(Attendee()))))
		  ...
		  reset(for {
		    ...
		    _ 					<- Done(join1, PERFORMED)
		    _ 					<- Abandon(invitee1)
		    attendee1 			<- Play3(Attendee(), operator2, meeting1)
		    _ 					<- LetWholeExtension(attendee1, "persistent", true)
		  } yield ())
		
		  obtained should be(getState())
		}

		it("Join Attendee in a rescheduled meeting") {
		  ...
		  val Output(meeting1, operator2) = reset(for {
		    ...
		    meeting1 			<- Initiate2(Meeting(_persistent = true, 
		    									...
		    									_room = "3",
		    									_reason = "Reason1",
		    									_substatus = Some(Rescheduled)), project1)
		    moderator1 			<- Play3(Moderator(_persistent = true), admin1, meeting1)
		    invitee1 			<- Play3(Invitee(_persistent = true), operator2, meeting1)
		  } yield (meeting1, operator2))
		...
		it("Join Attendee in a completed meeting") {
		...
		it("Join Attendee without invitee") {
		  ...
		  attempt(Say(operator2, meeting1, JoinAttendee(__new = Some(Attendee())))) should be(None)
