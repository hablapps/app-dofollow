...
class LeaveAttendeeTest(System: speech.System with DoFollowSystem with react.Debug) extends FunSpec with ShouldMatchers with BeforeAndAfter {
	describe( "LeaveAttendeeTest"){
		it("Leave Attendee") {

		  import System._
		
		  val Output(meeting1, attendee1) = reset(for {
		  ...
		  } yield (meeting1, attendee1))
		
		  val NextState(obtained) = attempt(Say(attendee1, meeting1, LeaveAttendee()))
		  ...
		  reset(for {
		    ...
		    moderator1 			<- Play3(Moderator(_persistent = true), admin1, meeting1)
		    attendee1 			<- Play3(Attendee(_persistent = true), operator1, meeting1)
		    leave1 				<- Say(attendee1, meeting1, LeaveAttendee())
