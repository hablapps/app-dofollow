...
class SetUpMeetingTest(System: speech.System with DoFollowSystem with react.Debug) extends FunSpec with ShouldMatchers with BeforeAndAfter {
	describe( "SetUpMeetingTest"){
		it("SetUp Meeting") {
		  ...
		  val NextState(obtained) = attempt(Say(admin1, project1, SetUpMeeting(__new = Some(Meeting(_persistent = true, 
		  										_date = 123,
		    									_room = "3",
		    									_reason = "Reason1",
		    									_substatus = Some(Scheduled))))))
		  ...
		  reset(for {
		    ...
		    _ 					<- Let(dep1.subinteraction += task1)
		    responsible1 		<- Play3(Responsible(_persistent = true), admin1, task1)
		    setUp 				<- Say(admin1, project1, SetUpMeeting(__new = Some(Meeting(_persistent = true, 
		  										_date = 123,
		    									_room = "3",
		    									_reason = "Reason1",
		    									_substatus = Some(Scheduled)))))
